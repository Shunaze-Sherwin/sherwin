...
// Kiểm tra thế kẹt "khối vuông 2x2": nếu hộp (tại p, sau khi đẩy) cùng 3 ô còn lại
// của một hình vuông 2x2 nào đó đều là tường hoặc hộp khác, thì không hộp nào trong
// khối đó còn có thể di chuyển được nữa (mọi hướng đẩy đều cần đứng/đẩy vào chính các
// ô đã bị chiếm trong khối). Đây là luật deadlock chuẩn, không phụ thuộc đích ở đâu,
// nên không bao giờ gây false-positive (chỉ có thể bỏ sót thế kẹt phức tạp hơn, không
// bao giờ chặn nhầm một nước đi thực sự còn khả thi).
bool isFrozenSquare(const Grid& g, Pos p, const vector<Pos>& otherBoxes) {
    if (g.cell[p.r][p.c] == 'A' || g.cell[p.r][p.c] == 'B') return false;

    auto occupied = [&](Pos q) {
        if (g.isWall(q)) return true;
        for (Pos b : otherBoxes) if (b == q) return true;
        return false;
    };
