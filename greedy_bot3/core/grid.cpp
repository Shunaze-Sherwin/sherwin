...
// Mặt nạ ô bị chiếm (hộp, nhân vật đối phương...) — dùng cho BFS đường đi của agent
typedef array<array<bool, 16>, 16> Occupancy;

inline Occupancy emptyOccupancy() {
    Occupancy occ{};
    for (int r = 0; r < 16; ++r)
        for (int c = 0; c < 16; ++c) occ[r][c] = false;
    return occ;
}
